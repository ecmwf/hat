...
def mock_latitudes_longitudes():
    ...
    return np.array([0, 1, 2, 3, 4]), np.array([0, 1, 2, 3, 4])
...
def test_process_station_data(mock_station, mock_latitudes_longitudes, mock_nc_data):
    ...
    assert 'optimum_distance_km' in processed_data, "Optimum distance in kilometers is missing"
    assert isinstance(processed_data['optimum_distance_km'], float), "Optimum distance should be a float"
    ...
    if 'near_grid_lat_idx' in processed_data:
        assert isinstance(processed_data['near_grid_lat_idx'], int), "Near grid latitude index should be an integer"
        
    if 'near_grid_lon_idx' in processed_data:
        assert isinstance(processed_data['near_grid_lon_idx'], int), "Near grid longitude index should be an integer"
        
    if 'optimum_grid_lat_idx' in processed_data:
        assert isinstance(processed_data['optimum_grid_lat_idx'], int), "Optimum grid latitude index should be an integer"
        
    if 'optimum_grid_lon_idx' in processed_data:
        assert isinstance(processed_data['optimum_grid_lon_idx'], int), "Optimum grid longitude index should be an integer"
