...
def test_find_best_matching_grid():
    ...
    max_area_diff = 10
    lat_idx, lon_idx = find_best_matching_grid(lat, lon, latitudes, longitudes, nc_data, csv_value, max_neighboring_cells, max_area_diff)
    assert lat_idx == 3 and lon_idx == 3
...
def test_process_station_data():
    ...
    nc_data = np.full((5, 5), 0.5)  # Fixed mock data
    cell_size = 1
    processed_data = process_station_data(station, latitudes, longitudes, nc_data, 2, 10, 20, 'lat_col', 'lon_col', 'station_name_col', 'csv_variable', cell_size, None, None, None)
