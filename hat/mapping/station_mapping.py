...
def main(config):
    ...
    stations_filter = config["stations_filter"]
    ...
    stations = hat.observations.read_station_metadata_file(
        csv_file,
        [lon_col, lat_col],
        stations_epsg,
        stations_filter,
    ...
    cell_size_check = abs(latitudes[0] - latitudes[1])
    assert cell_size == cell_size_check, "Grid cell size does not match the specified arc minute size"
    ...
    df['line'] = df.apply(lambda row: LineString([(row['station_lon'], row['station_lat']), (row['near_grid_lon'], row['near_grid_lat'])]), axis=1)

    # create line between station and new grid
    df['line_new'] = df.apply(lambda row: LineString([(row['station_lon'], row['station_lat']), (row['new_grid_lon'], row['new_grid_lat'])]), axis=1)
    ...
    gdf_line = gpd.GeoDataFrame(df, geometry=df['line'])
    gdf_line_new = gpd.GeoDataFrame(df, geometry=df['line_new'])
